<script>
	import IconMedal from '$lib/assets/icons/IconMedal.svelte'
</script>

<div class="mx-auto max-w-7xl py-20 font-urban">
	<section class="">
		<h2
			class="flex flex-wrap items-center justify-center gap-2 text-center font-urban text-4xl font-semibold leading-normal text-white max-md:flex-col md:text-5xl">
			Become an Affiliate
		</h2>

		<p class="mt-4 text-balance text-center text-white/60 md:mt-6 md:text-lg">
			Bring your contract from another platform and spend/earn at least $10,000 to earn an Affiliate
			badge (which provides higher rank in jobs and bids, and the lowest fees on future services)
		</p>

		<div class="relative mx-auto mt-20 aspect-square w-64 bg-white/10 p-2">
			<img
				class="h-full w-full object-cover"
				src="https://plus.unsplash.com/premium_photo-1678197937465-bdbc4ed95815?q=80"
				alt="a freelancer" />

			<div class="absolute -bottom-8 -right-8">
				<IconMedal />
			</div>
		</div>
	</section>
</div>
